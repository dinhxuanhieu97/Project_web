...
productContainer.addEventListener("click", function (e) {
  if (e.target.classList.contains("product-photo")) {
  ...
  if (
    ...
    e.target.parentElement.classList.contains("btn-like")
  ) {
    console.log("aaaa");
    const id = e.target.value || e.target.parentElement.value;
    const t = e.target.classList.contains("btn-like")
      ? e.target
...
async function getProductById(id) {
  ...
  document.querySelector(
    `#view-${id}`
  ).innerHTML = `<i class="bi bi-eye-fill"></i> ${result.views}`;
}
...
btnCategories.forEach((element) => {
  element.addEventListener("click", function () {
    ...
    checkedCategories = [...checkedCat].map((e) => {
      return e.value;
    });
...
async function getProductByCategories(id) {
  ...
  result.forEach((element) => {
    productContainer.innerHTML += `
        <div class="col-md-3">
            <img src="./public/images/${element.image}" alt="" class="img-fluid product-photo" data-product-id="${element.id}" data-bs-toggle="modal" data-bs-target="#productModal">
            ...
            <button class="btn badge text-bg-danger btn-like" value="${element.id}"><i class="bi bi-heart-fill"></i> ${element.likes}</button>
